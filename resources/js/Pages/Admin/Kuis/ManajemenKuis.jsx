...
const TYPE_CONFIG = {
    ...
    fill_blank: { label: 'Mengetik/Isian', color: 'bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300' },
    typing: { label: 'Mengetik/Isian', color: 'bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-300' },
...
export default function QuizzesIndex({ quizzes, modules = [], filters = {} }) {
    ...
    const handleUpdateQuiz = (e) => {
        ...
        editForm.put(route('admin.quizzes.update', editingQuiz.id), {
            preserveScroll: true,
        ...
        <AuthenticatedLayout>
                                        ...
                                        <tr key={quiz.id} className="transition-colors hover:bg-gray-50/70 dark:hover:bg-gray-800/40">
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <button onClick={() => openEditModal(quiz)} className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-xs font-black text-gray-600 transition-colors hover:border-[#E64A19] hover:text-[#E64A19] dark:border-gray-700 dark:text-gray-300">
                            ...
                            {filtered.map((quiz) => (
                                <div key={quiz.id} className="rounded-2xl border border-gray-100 p-4 dark:border-gray-800">
                                    ...
                                    <div className="mt-4 flex flex-wrap justify-end gap-2">
                                        ...
                                        <button onClick={() => openEditModal(quiz)} className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-xs font-black text-gray-600 dark:border-gray-700 dark:text-gray-300"><EditOutlinedIcon sx={{ fontSize: 15 }} /> Edit</button>
            ...
            {editingQuiz && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl dark:bg-gray-900">
                        <div className="border-b border-gray-100 p-6 dark:border-gray-800">
                            <h3 className="text-lg font-black text-gray-900 dark:text-white">Edit Kuis</h3>
                            <p className="mt-1 text-sm text-gray-400 dark:text-gray-500">
                                Atur ulang modul tujuan, tipe kuis, waktu, nilai lulus, dan status publish.
                            </p>
                        </div>
                        <form onSubmit={handleUpdateQuiz} className="space-y-4 p-6">
                            <div className="rounded-2xl bg-gray-50 p-4 dark:bg-gray-800/50">
                                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Konteks Saat Ini</p>
                                <p className="mt-1 text-sm font-black text-gray-900 dark:text-white">{editingQuiz.module?.title || 'Modul tidak ditemukan'}</p>
                                <p className="mt-1 text-xs font-bold text-gray-500 dark:text-gray-400">
                                    Quiz #{editingQuiz.id} - {editingQuiz.question_count || 0} soal
                                </p>
                            </div>
                            <div>
                                <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Modul Mingguan</label>
                            ...
                            <div>
                                <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Tipe Kuis</label>
                                <select value={editForm.data.type} onChange={(e) => editForm.setData('type', e.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:text-white" required>
                                    <option value="multiple_choice">Pilihan Ganda</option>
                                    <option value="fill_blank">Mengetik/Isian</option>
                                    <option value="listening">Mendengarkan</option>
                                </select>
                                {editForm.errors.type && <p className="mt-1 text-xs font-bold text-red-600">{editForm.errors.type}</p>}
                            </div>
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Batas Waktu (detik)</label>
                                    <input type="number" min="0" value={editForm.data.time_limit} onChange={(e) => editForm.setData('time_limit', e.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:text-white" placeholder="Kosongkan jika tanpa batas" />
                                    {editForm.errors.time_limit && <p className="mt-1 text-xs font-bold text-red-600">{editForm.errors.time_limit}</p>}
                                </div>
                                <div>
                                    <label className="mb-1.5 block text-sm font-bold text-gray-700 dark:text-gray-300">Nilai Lulus (%)</label>
                                    <input type="number" min="1" max="100" value={editForm.data.passing_score} onChange={(e) => editForm.setData('passing_score', e.target.value)} className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 dark:border-gray-700 dark:bg-gray-950 dark:text-white" placeholder="Default 70" />
                            ...
                            <div>
                                ...
                                {editForm.errors.status && <p className="mt-1 text-xs font-bold text-red-600">{editForm.errors.status}</p>}
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={closeEditModal} className="h-11 flex-1 rounded-xl border border-gray-200 text-sm font-bold text-gray-600 dark:border-gray-700 dark:text-gray-400">Batal</button>
                                <button type="submit" disabled={editForm.processing} className="h-11 flex-1 rounded-xl bg-[#E64A19] text-sm font-bold text-white">{editForm.processing ? 'Menyimpan...' : 'Simpan Perubahan'}</button>
                            </div>
                        </form>
                    </div>
                </div>
